...
public class GameMaster : MonoBehaviour
{
    ...
    public void Reset(int level)
    {
        ...
        for (int i = 0; i < scoreMeterStars.Length; i++)
        {
            if (i > 2)//ConfigReader.Instance.getValueInt("StarsLevel" + currentLevel) - 1)
            {
    ...
    void Update()
    {
        ...
        if(speedUpTime < Time.time)
        {
            ...
            speedUp();
            speedUpTime = Time.time + GlobalVariables.Instance.GetCrystalSpeedLevel(currentLevel);// ConfigReader.Instance.getValueInt("CrystalSpeedLevel" + currentLevel);
            for (int i = 0; i < lanetextscript.Length; i++)
            {
        ...
        if (spawnRateTime < Time.time)
        {
            if (AM.abilityActive && (AM.abilitysInLane[0].y == 1))
            {
                spawnRateTime = Time.time + GlobalVariables.Instance.GetCrystalSpawnRateLevel(currentLevel);// ConfigReader.Instance.getValueInt("SpawnRateLevel"+currentLevel);
                return;
            }
            spawnrateUp();
            spawnRateTime = Time.time + GlobalVariables.Instance.GetCrystalSpawnRateLevel(currentLevel);// ConfigReader.Instance.getValueInt("SpawnRateLevel" + currentLevel);
        ...
        if ((blueScore + redScore) > GlobalVariables.Instance.GetStarReq(currentLevel)-1)//ConfigReader.Instance.getValueInt("StarRequirementLevel" + currentLevel)-1)
            scoreMeterStars[0].SetActive(true);
        if ((blueScore + redScore) > (GlobalVariables.Instance.GetStarReq(currentLevel)*2) - 1) //(ConfigReader.Instance.getValueInt("StarRequirementLevel" + currentLevel)*2)-1)
                scoreMeterStars[1].SetActive(true);
        if ((blueScore + redScore) > (GlobalVariables.Instance.GetStarReq(currentLevel)*3) - 1)//(ConfigReader.Instance.getValueInt("StarRequirementLevel" + currentLevel)*3)-1)
